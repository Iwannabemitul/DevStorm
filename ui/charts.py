"""Radar + fulfillment bar chart. Unchanged visually from Round 1 -- only
relocated out of app.py and taught to read a plain `skill_proficiency` dict
(same shape as before: lowercased skill name -> weight)."""
...
def _proficiency_label(value: float) -> str:
    labels = {
        0.0: "None (0%)",
        0.4: "Beginner (40%)",
        0.8: "Intermediate (80%)",
...
def render_radar_chart(
    ...
    for requirement in required_skills:
        categories.append(requirement.split("/")[0].strip())
    ...
    current_levels_closed = [_proficiency_label(value) for value in user_values_closed]
